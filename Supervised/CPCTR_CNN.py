...
CPCTR = pd.read_csv("./CNN_CPCTR_dataset.csv")
my_output = None
...
def load_clinical(id_train, id_validation):
    CPCTR_xls = pd.read_csv('./Outcome_TMA5_with_kattan_capra_all_patients.csv')
    dic_CPCTR = {}
    SV = {"No": 0, "Yes": 1, "Unknown": 2, "": 2}
    LN = {"No": 0, "Yes": 1, "Unknown":2}
    EE = {"None": 5, "Focal": 1, "Multifocal":2, "Established":3, "Unknown":4}
    surgical_margin = {"All surgical margins are free of tumor": 0, "Tumor focal at margin":1, "Tumor widespread at margin":2,
                    "Unknown": 3, "": 4}
    for i in range(len(CPCTR_xls["SURGICAL_MARGIN"])):
        l = []
        # The best result was acheived with 3 clinical features (we didn't add all of them)
        l.append(CPCTR_xls["GLEASON_PRIMARY_GRADE"][i])
        l.append(CPCTR_xls["GLEASON_SECONDARY_GRADE"][i])
        l.append(EE[CPCTR_xls["EXTRACAPSULAR_EXTENSION"][i]])
        
        if all_clinical_features: 
            l.append(CPCTR_xls["PSA_LEVEL"][i])
            l.append(LN[CPCTR_xls["LYMPH_NODE"][i]])
            try:
                l.append(SV[CPCTR_xls["SEMINAL_VESICLE"][i]])
            except KeyError:
                l.append(2)
            try:
                l.append(surgical_margin[CPCTR_xls["SURGICAL_MARGIN"][i]])
            except KeyError:
                l.append(4) 
        try:
            dic_CPCTR[str(int(CPCTR_xls["IMS_CASE_IDENTIFIER"][i]))] = [a*0.001 for a in l]
        except ValueError:
            print(CPCTR_xls["IMS_CASE_IDENTIFIER"][i])
...
def open_images(image_names, labels, case_ids):
    ...
    for i in range(len(image_names)):
        im = Image.open("./ALL/" + image_names[i] + "-" + str(bool(labels[i])) + "-" + str(int(case_ids[i])) + ".tif")
