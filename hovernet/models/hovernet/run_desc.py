...
from .utils import crop_to_shape, dice_loss, mse_loss, msge_loss, xentropy_loss, panoptic_quality
...
#     pq = panoptic_quality(true_np_onehot, pred_dict["np"])
#     track_value("loss_%s_%s" % ("np", "pq"), pq.cpu().item())
#     print("--pq:",spq)    
...
def infer_step(batch_data, model):
    ...
    model.module.conv_bot.register_forward_hook(get_activation('Conv2d'))
    model.module.decoder.tp.u0.conv.register_forward_hook(get_activation('Conv2d_tp'))
    model.module.decoder.np.u0.conv.register_forward_hook(get_activation('Conv2d_np'))
    model.module.decoder.hv.u0.conv.register_forward_hook(get_activation('Conv2d_hv'))
    ...
    model.eval()  # infer mode
    print("+++++++++++++++++++++++++========================")
    print("here in infer_step...")
    print("+++++++++++++++++++++++++========================")
    # --------------------------------------------------------------
    with torch.no_grad():  # dont compute gradient
        pred_dict = model(patch_imgs_gpu)
        print("((((((((((((conv))))))))))")
        conv_layer = activation['Conv2d'].cpu().numpy()
        conv_layer_tp = activation['Conv2d_tp'].cpu().numpy()
        conv_layer_np = activation['Conv2d_np'].cpu().numpy()
        conv_layer_hv = activation['Conv2d_hv'].cpu().numpy()
        print("pay attention: ")
        print(conv_layer.shape)
        print(conv_layer_tp.shape)
        print(conv_layer_np.shape)
        print(conv_layer_hv.shape)
    ...
    print("here in infer_step...")
    print("+++++++++++++++++++++++++========================")
    print(pred_output.cpu().numpy().shape)
    # * Its up to user to define the protocol to process the raw output per step!
...
def proc_valid_step_output(raw_data, nr_types=None):
    ...
    if nr_types is not None:
        ...
        print("pred_tp, true_tp size:", len(raw_data["true_np"]))
        for type_id in range(0, nr_types):
            ...
            for idx in range(len(raw_data["true_np"])):
                ...
                print("shape:m", np.array(patch_true_tp).shape)
    ...
    print(">>>>>>", acc_np, dice_np, mse)
    
    
    # *
    imgs = raw_data["imgs"]
    print("imgs:", np.array(imgs[0]).shape)
    ...
    print("viz_fig: ", np.array(viz_fig[0]).shape)
    return track_dict
